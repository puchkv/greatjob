...
<section>
	<div class = "content">
			...
			<div class = "user__profile">
				<span class = "user__profile__name">
					...
					<a href = "/profile/change">
						<div class = "user__profile__change"></div>
					</a>
				</span>
		...
		@if($user->isPerformer)
			@if($works != null)
			@if($works->count() > 0)
				<span class = "title">Ви виконуєте</span>
				<ul class = "user__tasks">
				@foreach($works as $work)
					<li>
						<span style="color:#777; font-size: 12px">№{{ $work->id }}</span>
						<a href="/task/{{ $work->id }}">
							{{ $work->title }}
						</a>
						<span style="color:#777; font-size: 12px">({{ $work->getStatusName($work->status) }})</span>
						<span class = "user__tasks__date">{{ $work->created_at->isoFormat('Do MMMM YYYY в HH:mm') }}</span>
					</li>
				@endforeach
				</ul>
			@endif
		...
		@endif
			...
			<div class = "content__info__right">
				<div class = "user__progress">
					...
					<div class = "user__rating">Ваш рейтинг</div>
					<div class = "user__reviews">Ви маєте {{ round($user->rating) }}% позитивних відгуків</div>
					@if(!$user->isPerformer)
						<button onclick="location.href = '/register/performer'">Стати виконавцем</button>
					@endif
				</div>
			</div>
		</div>
